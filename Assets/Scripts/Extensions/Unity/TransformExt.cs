...
namespace Extensions.Unity
{
    public static class TransformExt
    {
        ...
        public static void GetComponentsInChildrenRecursive<T>(this Transform trans, List<T> components)
        {
            for (int i = 0; i < trans.childCount; i ++)
            {
                ...
                if (thisChild.TryGetComponent(out T component))
                {
                    ...
                    thisChild.GetComponentsInChildrenRecursive(components);
                }
                else
                {
                    thisChild.GetComponentsInChildrenRecursive(components);
        ...
        /// <summary>
        /// Includes current Transform`s GameObject
        /// </summary>
        /// <param name="trans"></param>
        /// <param name="component"></param>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public static bool TryGetComponentInChildrenRecursive<T>(this Transform trans, out T component) where T : Component
        {
            component = null;

            if(trans.TryGetComponent(out component)) return true;
            
            for (int i = 0; i < trans.childCount; i ++)
            {
                Transform thisChild = trans.GetChild(i);
        
                if (thisChild.TryGetComponent(out component))
                {
                    return true;
                }

                if(trans.GetChild(i).TryGetComponentInChildrenRecursive(out component))
                {
                    return true;
                }
            }
