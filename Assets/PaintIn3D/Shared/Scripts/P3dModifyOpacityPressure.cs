...
namespace PaintIn3D
{
	...
	public class P3dModifyOpacityPressure : P3dModifier
	{
		...
		{
			opacity = EditorGUILayout.FloatField(new GUIContent("Opacity", "The paint component's Opacity value will be modified using this value based on the current Blend setting."), opacity);
			blend = (BlendType)EditorGUILayout.EnumPopup(new GUIContent("Blend", "This allows you to control how this new Opacity value will modify the old value in the paint component.\n\nReplace = Transition between [old, new] based on pressure.\n\nMultiply = Transition between [old, old*new] based on pressure.\n\nIncrement = Transition between [old, old+new] based on pressure."), blend);
		}
#endif
	}
}