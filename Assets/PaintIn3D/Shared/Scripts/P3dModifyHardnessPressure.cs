...
namespace PaintIn3D
{
	...
	public class P3dModifyHardnessPressure : P3dModifier
	{
		...
		{
			hardness = EditorGUILayout.FloatField(new GUIContent("Hardness", "The paint component's Hardness value will be modified using this value based on the current Blend setting."), hardness);
			blend = (BlendType)EditorGUILayout.EnumPopup(new GUIContent("Blend", "This allows you to control how this new Hardness value will modify the old value in the paint component.\n\nReplace = Transition between [old, new] based on pressure.\n\nMultiply = Transition between [old, old*new] based on pressure.\n\nIncrement = Transition between [old, old+new] based on pressure."), blend);
		}
#endif
	}
}