...
namespace PaintIn3D
{
	...
	public class P3dModifyHardnessRandom : P3dModifier
	{
		...
		{
			min = EditorGUILayout.FloatField(new GUIContent("Min", "This is the minimum random hardness that will be picked."), min);
			max = EditorGUILayout.FloatField(new GUIContent("Max", "This is the maximum random hardness that will be picked."), max);
			blend = (BlendType)EditorGUILayout.EnumPopup(new GUIContent("Blend", "The way the picked hardness value will be blended with the current one."), blend);
		}
#endif
	}
}