...
namespace PaintIn3D
{
	...
	public class P3dModifyOpacityRandom : P3dModifier
	{
		...
		{
			min = EditorGUILayout.FloatField(new GUIContent("Min", "This is the minimum random opacity that will be picked."), min);
			max = EditorGUILayout.FloatField(new GUIContent("Max", "This is the maximum random opacity that will be picked."), max);
			blend = (BlendType)EditorGUILayout.EnumPopup(new GUIContent("Blend", "The way the picked opacity value will be blended with the current one."), blend);
		}
#endif
	}
}